document.addEventListener('DOMContentLoaded', function() {
  ...
  function isAccountLocked(email) {
    const lockoutUntil = sessionStorage.getItem(getAccountLockoutKey(email));
  ...
  function handleFailedAttempt(email) {
    ...
    let attempts = parseInt(sessionStorage.getItem(storageKey)) || 0;
    attempts++;
    sessionStorage.setItem(storageKey, attempts.toString());
    ...
    if (attempts >= MAX_ATTEMPTS_PER_ACCOUNT) {
      ...
      sessionStorage.setItem(getAccountLockoutKey(email), lockoutUntil.toString());
  ...
  function resetFailedAttempts(email) {
    sessionStorage.removeItem(getAccountStorageKey(email));
  ...
  usuarioInput.addEventListener('input', function() {
    ...
    if (email) {
      const attempts = parseInt(sessionStorage.getItem(getAccountStorageKey(email))) || 0;
      updateAttemptsWarning(email, attempts);
    } else {
      attemptsWarning.style.display = 'none';
    }
