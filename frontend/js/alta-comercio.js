...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Event listener para botón "Eliminar" del primer anexo
    const btnEliminarAnexo1 = document.getElementById('btnEliminarAnexo1');
    if (btnEliminarAnexo1) {
        btnEliminarAnexo1.addEventListener('click', function () {
            removeAnexo(1);
...
// Hacer la función disponible en scope global para onclick en HTML
...
function manejarCargaDocumento(input) {
    ...
    // Buscar el contenedor más cercano (.documento-item) para encontrar el fileInfo
    const docItem = input.closest('.documento-item');
    const fileInfo = docItem?.querySelector('.file-info');
    ...
    function openPreviewModal(url, ctx) {
        ...
        // Eliminar desde el MODAL (X roja) - usar delegación de eventos
        overlay.addEventListener('click', handleModalClose, true);
        ...
        function handleModalClose(e) {
            ...
            if (e.target === btnX) {
                e.stopPropagation();
                e.preventDefault();
                const { inputRef, wrapperRef, fileInfoRef } = ctx || {};
                ...
                overlay.removeEventListener('click', handleModalClose, true);
            }
        }
    }
    ...
    if (file) {
        ...
        if (/^image\//.test(file.type)) {
            ...
            // Usar FileReader.readAsDataURL() en lugar de blob URLs (CSP permite data:)
            ...
            reader.onload = function(e) {
                ...
                img.addEventListener('click', () => {
                    openPreviewModal(dataUrl, { inputRef: input, wrapperRef: wrapper, fileInfoRef: fileInfo });
                });
            };
...
async function enviarFormulario() {
    try {
        ...
        // ✅ Mostrar notificación inicial CON Nº de Habilitación
        window.alert(`El comercio ha sido dado de alta exitosamente.\n\nNº de Habilitación: ${result.idComercio}`, 'success');
  ...
  `;
                ...
                img.src = dataQR.qr_path; // PNG servido como ruta pública /uploads/qr/
