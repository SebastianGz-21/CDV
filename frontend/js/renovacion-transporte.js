...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateBtnVolverRenovacionTransporte() {
        const btnVolver = document.getElementById('btnVolverRenovacionTransporte');
        const origin = sessionStorage.getItem('originFrom') || 'bromatologia';
        
        if (origin === 'lista-transportes') {
            btnVolver.href = './lista-transportes.html';
            btnVolver.title = 'Volver al listado de transportes';
        } else {
    ...
    // 🆕 Detectar si viene de lista-transportes (con parámetro id en URL)
    const urlParams = new URLSearchParams(window.location.search);
    const transporteIdDesdeURL = urlParams.get('id');
    
    // Si viene de lista-transportes, marcar el origen automáticamente
    if (transporteIdDesdeURL) {
      sessionStorage.setItem('originFrom', 'lista-transportes');
    }

    // Actualizar botón de volver al cargar
    updateBtnVolverRenovacionTransporte();
    
    // Si viene de lista-transportes, llenar el campo y buscar automáticamente
    if (transporteIdDesdeURL) {
      idTransporte.value = transporteIdDesdeURL;
      // Marcar para buscar después de que se carguen todos los elementos
      window._buscarAlCargar = true;
    }

    // --- Mostrar modal de advertencia al ingresar (solo si NO viene de lista-transportes) ---
    const modalEl = document.getElementById('modalAdvertencia');
    if (modalEl && !transporteIdDesdeURL) {
    ...
    const camposBloqueados = document.querySelectorAll(
        'input, select, textarea, button:not(#btnBuscarTransporte):not(#btnAceptarModalAdvertencia):not(#btnAceptarModalNaranja)'
    );
  ...
  if (reemplazosDocs.has(key)) {
    ...
    if (!doc.ruta_archivo || doc.faltante) {
      ...
      vista.src = pathPublico(doc.ruta_archivo || doc.archivo || '');
    }
  ...
  inp.click();
    ...
    // 🆕 Si viene de lista-transportes, buscar automáticamente después de cargar
