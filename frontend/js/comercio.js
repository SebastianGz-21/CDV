...
async function guardarCambios() {
  try {
    ...
    cargarDatosComercio(comercioOriginal, userDataGlobal);
    toggleModoEdicion();
  }
}

// Función para cargar los datos del comercio
function cargarDatosComercio(comercio, userData = null) {
  safeSetContent('numero-habilitacion', comercio.id_comercio);
  ...
  const esInspector = userData && (userData.rol?.toLowerCase() === 'inspector' || userData.role?.toLowerCase() === 'inspector');
  ...
  console.log('Condiciones inspección - DETALLADO:', {
    ...
    rol_usuario: userData?.rol || userData?.role,
    mostrarBoton: pendienteInspeccion && esInspector
  });
  
  if (pendienteInspeccion && esInspector) {
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    // 5. Mostrar datos
    cargarDatosComercio(comercio, userData);
    ...
    if (userData.rol === 'inspector') {
      ...
      if (btnCancelar) {
        btnCancelar.addEventListener('click', () => {
          cargarDatosComercio(comercioOriginal, userData);
          toggleModoEdicion();
        });
      }
