...
limpiarDocumentacionUI = () => {
  const contenedor = document.getElementById('contenedorDocComercio');
  if (!contenedor) return;
  contenedor.innerHTML = `<div class="row g-4 justify-content-start"></div>`;
      ...
      inp.onchange = (e) => {
        ...
        if (file.type.startsWith('image/')) {
          ...
          reader.onload = function(e) {
            vista.src = e.target.result; // data URL permitido en CSP
          };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  import('../auth.js').then(({ checkAuth, infoUsuario }) => {
    const userData = checkAuth();
    if (!userData) return;
    
    infoUsuario(userData);
  }).catch(e => console.warn('No se pudo cargar auth.js', e));
  ...
  function updateBtnVolverRenovacionComercio() {
    const btnVolver = document.getElementById('btnVolverRenovacionComercio');
    const origin = sessionStorage.getItem('originFrom') || 'bromatologia';
    
    if (origin === 'lista-comercios') {
      btnVolver.href = './lista-comercios.html';
      btnVolver.title = 'Volver al listado de comercios';
    } else {
      btnVolver.href = './bromatologia.html';
      btnVolver.title = 'Volver a Bromatología';
    }
  }

  // 🆕 Detectar si viene de lista-comercios (con parámetro id en URL)
  const urlParams = new URLSearchParams(window.location.search);
  const comercioIdDesdeURL = urlParams.get('id');
  
  // Si viene de lista-comercios, marcar el origen automáticamente
  if (comercioIdDesdeURL) {
    sessionStorage.setItem('originFrom', 'lista-comercios');
  }

  // Actualizar botón de volver al cargar
  updateBtnVolverRenovacionComercio();
  
  // Si viene de lista-comercios, llenar el campo y buscar automáticamente
  if (comercioIdDesdeURL) {
    idComercio.value = comercioIdDesdeURL;
    // Marcar para buscar después de que se carguen todos los elementos
    window._buscarAlCargar = true;
  }
  ...
  // --- Mostrar modal de advertencia al ingresar (solo si NO viene de lista-comercios) ---
  const modalEl = document.getElementById('modalAdvertencia');
  if (modalEl && !comercioIdDesdeURL) {
  ...
  const camposBloqueados = document.querySelectorAll(
    'input, select, textarea, button:not(#btnBuscarComercio):not(#btnAceptarModalAdvertencia):not(#btnAceptarModalNaranja)'
  );
  ...
  // 🆕 Si viene de lista-comercios, buscar automáticamente después de cargar
  if (window._buscarAlCargar) {
    // Dar tiempo a que se carguen todos los elementos del DOM
    setTimeout(() => {
      buscarComercio();
    }, 100);
  }
