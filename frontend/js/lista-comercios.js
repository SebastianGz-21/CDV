...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Ordenar comercios por número de habilitación descendente (más recientes primero)
  function ordenarComercios(datos) {
    return datos.sort((a, b) => {
      const numA = parseInt(a.id_comercio) || 0;
      const numB = parseInt(b.id_comercio) || 0;
  ...
  function renderizarTabla() {
    ...
    // Obtener datos para la página actual (siempre paginar)
    const datosPaginados = obtenerDatosPaginados();
    ...
    datosPaginados.forEach(comercio => {
      ...
      fila.innerHTML = `
        <td><strong>${comercio.id_comercio || '-'}</strong></td>
        <td>${comercio.nombre_comercial}</td>
        <td>${comercio.titular}</td>
        <td>${comercio.categoria}</td>
        <td>${comercio.rubro}</td>
  ...
  function actualizarPaginacion() {
    ...
    document.getElementById('btn-anterior').disabled = paginaActual <= 1;
    document.getElementById('btn-siguiente').disabled = paginaActual >= totalPaginas;
    // Actualizar cantidad de comercios
    document.getElementById('totalComercios').textContent = `Cantidad de comercios: ${totalRegistros}`;
  }
  ...
  async function aplicarFiltros() {
    ...
    if (!estadoFiltro && !categoriaFiltro && !rubroFiltro && !busquedaFiltro && !nombreFiltro) {
    ...
    if (busquedaFiltro) {
      comerciosFiltrados = comerciosFiltrados.filter(comercio => 
        (comercio.id_comercio && comercio.id_comercio.toString().includes(busquedaFiltro))
      );
    }

    if (nombreFiltro) {
      comerciosFiltrados = comerciosFiltrados.filter(comercio => 
        comercio.nombre_comercial.toLowerCase().includes(nombreFiltro) ||
        comercio.titular.toLowerCase().includes(nombreFiltro)
      );
    }

    comercios = ordenarComercios(comerciosFiltrados);
    totalRegistros = comerciosFiltrados.length;
    paginaActual = 1;
    usandoFiltros = true;
    renderizarTabla();
  ...
  async function cargarDatosIniciales() {
    const response = await fetchTodosComercios();
    comercios = ordenarComercios(response.data);
    totalRegistros = response.total;
    paginaActual = 1;
    usandoFiltros = false;
    renderizarTabla();
  ...
  // Guardar el origen cuando venimos de bromatologia o panel
  function setVolverOrigin() {
    const referrer = document.referrer;
    if (referrer.includes('bromatologia')) {
      sessionStorage.setItem('originFrom', 'bromatologia');
    } else if (!referrer.includes('lista-comercios')) {
      sessionStorage.setItem('originFrom', 'panel-principal');
    }
  }

  // Actualizar el botón de volver según el origen
  function updateBtnVolverComercio() {
    const btnVolver = document.getElementById('btnVolverComercio');
    const origin = sessionStorage.getItem('originFrom') || 'panel-principal';
    
    if (origin === 'bromatologia') {
      btnVolver.href = '../pages/bromatologia.html';
  ...
  // Guardar origen cuando el usuario haga clic en "Renovar"
  function setupRenovarButtons() {
    // Esperar a que la tabla esté renderizada
    setTimeout(() => {
      const renovarButtons = document.querySelectorAll('button[data-id]');
      renovarButtons.forEach(btn => {
  ...
  setVolverOrigin();
  updateBtnVolverComercio();

  // Inicializar
  await cargarDatosIniciales();
  setupRenovarButtons();
  
  // Re-setup de botones después de renderizar
  const observer = new MutationObserver(() => {
    setupRenovarButtons();
  });
  const cuerpotabla = document.getElementById('cuerpo-tabla');
  if (cuerpotabla) {
    observer.observe(cuerpotabla, {
      childList: true,
      subtree: true
    });
  }
