...
exports.registrarComercioCompleto = async (req, res) => {
    try {
        ...
        // ✅ LÓGICA DE HABILITACIÓN AUTOMÁTICA POR TAMAÑO
        // - Si metros <= 20: HABILITADO INSTANTÁNEAMENTE (sin inspección ocular)
        // - Si metros > 20 o sin datos: PENDIENTE DE INSPECCIÓN
        ...
        // 🎯 DETERMINAR HABILITACIÓN AUTOMÁTICA
        // Si metros <= 20: habilitar instantáneamente, sino: pendiente de inspección
        const requiereInspeccion = !metrosDB || metrosDB > 20;
        
        let fechaHabilitacionDB = null;
        let fechaVencimientoDb = null;
        let pendienteInspeccionDB = 1;

        if (!requiereInspeccion) {
            // HABILITACIÓN AUTOMÁTICA: metros <= 20
            const hoy = new Date();
            fechaHabilitacionDB = hoy.toISOString().split('T')[0];
            const vencimiento = new Date(hoy);
            
            // Determinar vigencia según categoría
            if ((categoria || '').toLowerCase() === 'vendedor ambulante') {
                vencimiento.setMonth(vencimiento.getMonth() + 1);
            } else {
                vencimiento.setFullYear(vencimiento.getFullYear() + 1);
            }
            fechaVencimientoDb = vencimiento.toISOString().split('T')[0];
            pendienteInspeccionDB = 0; // ✅ YA HABILITADO
        }
...
`, [
            ...
            fechaVencimientoDb,
            activo,
            pendienteInspeccionDB
        ]);


        const idComercio = resultComercio.insertId;

        // 📋 LOGGING: Mostrar estado de habilitación
        if (!requiereInspeccion) {
            console.log(`✅ COMERCIO HABILITADO AUTOMÁTICAMENTE: ${nombreFinal} (${metrosDB} m² <= 20)`);
        } else {
            console.log(`⏳ COMERCIO PENDIENTE DE INSPECCIÓN: ${nombreFinal} (${metrosDB || 'sin datos'} m²)`);
        }
