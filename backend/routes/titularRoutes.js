...
// ===============================
// � Servir documento del titular con autenticación
// ===============================
router.get('/archivo/:filename', verificarToken, (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(__dirname, '../uploads/documentos_titular', nombreArchivo);
    const uploadsDir = path.join(__dirname, '../uploads/documentos_titular');
    
    // Verificar que la ruta está dentro de documentos_titular
    if (!rutaCompleta.startsWith(uploadsDir)) {
      return res.status(403).json({ error: 'Acceso denegado' });
    }
